...
void TCP_send_student_information(){
   ...
   if (connect(tcp_fd, (struct sockaddr *) &tcp_saddr, sizeof(tcp_saddr)) < 0) { //connecting to server
    ...
    int PortS5 = ntohs(tcp_caddr.sin_port);

    cout << "<Student 5> has TCP port " << PortS5 << " and IP address " << IPaddr << endl;

    string student5_information = readData("student5.txt");

    strcpy(buf, student5_information.c_str());
    if ((numbyte = send(tcp_fd, buf, MAXDATASIZE-1, 0)) > 0) {
        sleep(1);
    }

    cout << "Completed sending application for <" <<  student5_information.substr(0,8) << ">" << endl;
    sleep(1);
}
...
void UDP_recieve_application_result(){
   ...
   udp_saddr.sin_port = htons(Student5_UDP_PORT); // specify port to listen on
   ...
   cout << "<Student5> has UDP port " << Student5_UDP_PORT << " and IP address " << IPaddr << endl;

   char buf[MAXDATASIZE];

   socklen_t clilen = sizeof(udp_fd);
   recvfrom(udp_fd, buf, MAXDATASIZE, 0, (struct sockaddr *) &udp_caddr, &clilen);
   cout << "<Student5> has received the application result, the result is:" << buf << endl;
   sleep(1); 
   close(udp_fd);
}
