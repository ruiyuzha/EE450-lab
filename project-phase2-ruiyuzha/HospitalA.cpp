...
void TCP_send_hospital_information(){
   ...
   if (connect(tcp_fd, (struct sockaddr *) &tcp_saddr, sizeof(tcp_saddr)) < 0) { //connecting to server
    ...
    cout << "<Hospital A> has TCP port " << PortHA << " and IP address " << IPaddr << " for Phase 1" << endl;
...
void UDP_recieve_admitted_result(){
   ...
   udp_saddr.sin_port = htons(Hospital1_UDP_PORT); // specify port to listen on
   ...
   cout << "<HospitalA> has UDP port " << Hospital1_UDP_PORT << " and IP address " << IPaddr << " for Phase 2" << endl;

   char buf[MAXDATASIZE];

   socklen_t clilen = sizeof(udp_fd);
   recvfrom(udp_fd, buf, MAXDATASIZE, 0, (struct sockaddr *) &udp_caddr, &clilen);
   cout << string(buf).substr(0,8) << " has addmitted to HospitalA" << endl;
   sleep(1);
}
